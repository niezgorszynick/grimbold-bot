// rollTracker.js — persists weekly d20 rolls per Discord user in Google Sheets
const { getUserRollFromSheet, setUserRollInSheet } = require('./sheets');
...
// ─── public API ──────────────────────────────────────────────────────────────
async function getUserRoll(userId) {
  return await getUserRollFromSheet(userId, getWeekKey());
}

async function setUserRoll(userId, rollResult) {
  await setUserRollInSheet(userId, getWeekKey(), rollResult);
}
